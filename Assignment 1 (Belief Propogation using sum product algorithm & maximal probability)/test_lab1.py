...
# borrowed the test case from Sriram Srikanth e1132261.
@wrap_test
def test_factor_marginalize_2():
    my_factor = Factor(var=[1, 2, 3, 4],
                       card=[2, 2, 2, 2],
                       val=[0.05, 0.04, 0.03, 0.06,
                            0.02, 0.01, 0.02, 0.02,
                            0.03, 0.06, 0.05, 0.08,
                            0.04, 0.03, 0.06, 0.10])
    my_vars_to_marginalize_out = [1, 2]

    my_correct = Factor(var = [3,4],
                        card = [2,2],
                        val = [0.18, 0.07,0.22, 0.23])


    my_output = factor_marginalize(my_factor, my_vars_to_marginalize_out)

    assert my_output == my_correct, STR_OUTPUT_MISMATCH

@wrap_test
def test_factor_marginalize_2():
    my_factor = Factor(var=[2, 3],
                    card=[2, 3],
                    val=[0.4, 0.35, 0.6, 0.45, 0.0, 0.2])
    my_vars_to_marginalize_out = [1]

    my_correct = Factor(var=[2, 3],
                    card=[2, 3],
                    val=[0.4, 0.35, 0.6, 0.45, 0.0, 0.2])
...
def test_compute_joint_distribution():
    factors = [
        Factor(var=[1], card=[2], val=[0.6, 0.4]),
        Factor(var=[1, 2], card=[2, 2], val=[0.7,0.2,0.3,0.8]),
        Factor(var=[1, 3], card=[2, 2], val=[0.9,0.4,0.1,0.6])
    ]
    correct = Factor(var=[1,2,3],card=[2,2,2],val=[0.378,0.032,0.162,0.128,0.042,0.048,0.018,0.192])
    output = compute_joint_distribution(factors)
    assert correct == output, STR_OUTPUT_MISMATCH
